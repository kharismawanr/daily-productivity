...
import { Send, Zap, User, RefreshCw, ClipboardList, Info, HelpCircle } from "lucide-react";
...
export const HermesChatView: React.FC<HermesChatViewProps> = ({ onRefreshAll }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: "HERMES",
      text: "Selamat datang kembali, Yang Mulia Aris! ⚡ Saya Hermes Agent. Saya siap mendengarkan instruksi Anda harian seperti mencatat tugas baru, menyelesaikan tugas, atau meminta ringkasan (daily summary) harian.\n\nContoh yang bisa dicoba:\n👉 \"catat task review SLA vendor besok jam 15:00\"\n👉 \"task review SLA vendor selesai\"\n👉 \"minta briefing harian\"",
      timestamp: new Date()
    }
  ]);
  ...
  // Very simple and smart local parsing to simulate NLP on the client/backend
  const parseLocalCommand = (text: string) => {
    const rawText = text.toLowerCase().trim();

    // 1. Check for summary / briefing
    ...
    // e.g. "catat task meeting vendor besok jam 17:00"
    // e.g. "tambah task beli kopi"
    ...
    if (createMatch) {
      ...
      // Simple time resolver
      const today = new Date();
      let targetDate = new Date();

      // Check for deadline day
      ...
      // Check for time (e.g. "jam 15:00" or "jam 3 sore" or "jam 17")
      ...
      if (timeMatch) {
        ...
        if (period) {
          ...
          // If no period is specified and hour is small, assume afternoon/evening (e.g. jam 5 -> 17:00)
          hour += 12;
        }
      ...
      // Check for project association, e.g. "di project MRP Automation" or "proyek X"
      ...
      // Check for priority (e.g. "prioritas tinggi" or "urgent" or "prioritas low")
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (command.action === "summary") {
        ...
        // Post complete request to backend
        ...
        if (result.success) {
          ...
          onRefreshAll(); // Refresh other tabs
        } else {
          setMessages(prev => [...prev, {
            sender: "HERMES",
        ...
        // Post create request to backend
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] border border-zinc-800/80 bg-zinc-950 rounded-2xl overflow-hidden">
      ...
      <div className="border-b border-zinc-800 bg-zinc-900/40 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm">Hermes Agent Console</h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {messages.map((msg, idx) => {
          const isHermes = msg.sender === "HERMES";
          return (
            <div key={idx} className={`flex gap-3 max-w-[85%] ${isHermes ? "mr-auto" : "ml-auto flex-row-reverse"}`}>
              {/* Avatar */}
              <div className={`h-8 w-8 rounded-xl flex items-center justify-center border flex-shrink-0 font-bold text-xs ${
                ...
                {isHermes ? <Zap className="h-4 w-4" /> : <User className="h-4 w-4" />}
              </div>
              ...
              <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                ...
                {msg.text}
                <span className={`block text-[9px] mt-1.5 text-right ${isHermes ? "text-zinc-500" : "text-emerald-950/70"}`}>
        ...
        {sending && (
          <div className="flex gap-3 mr-auto max-w-[85%]">
            ...
            <div className="rounded-2xl px-4 py-3 bg-zinc-900/60 border border-zinc-800 text-zinc-400 text-sm italic">
              Hermes sedang menulis ke database...
            </div>
          </div>
        )}
        <div ref={chatEndRef} />
      ...
      {/* Quick Suggestions Helper Panel */}
      ...
      <div className="border-t border-zinc-800 bg-zinc-900/30 p-4">
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="Tulis pesan ke Hermes..."
            value={input}
